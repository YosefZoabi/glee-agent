...
while their \emph{behaviour} differed by ${\le}0.010$ of the pot; separating two
policies 0.25 rating points per game apart at $3\sigma$ needs
...
nowhere and had to be recovered from play: the valuation pool, a round cap that
pays both sides nothing, and the rating's own update rule. I give four offline
techniques that measured what the leaderboard could not, and a survivorship
failure mode that silently corrupted four analyses. Playing both tracks also
yields a well matched \emph{observational} human-versus-machine comparison, in
which agent and human read seller messages with no difference I can detect
(cell-standardised hit rates 0.742 vs.\ 0.741, 95\% CI $[-0.033,+0.032]$) and
separate almost entirely on the discipline to decline.
\end{abstract}
...
\tbf{0.734} where it says take everything~\cite{ochsroth1989}. (Appendix Tables~\ref{tab:breakeven} and~\ref{tab:spe}).
...
Five changes replicated, by which I mean they held at ${\ge}3\sigma$ or across two
...
per game in product-price units. $t$: Welch's, signed human${-}$agent, so
negative favours the agent; $n{=}455$--2{,}745 human, 13{,}180--59{,}603 agent.}
\label{tab:human}
...
\tbf{(1) No detectable difference in reading sellers.} The pooled hit rate on
purchases is 0.738 over 2{,}745 for me and 0.725 over 59{,}603 for the agent, but
that gap is cell mix: standardised over the 13 shared $(m,p)$ cells it is
\tbf{0.742 against 0.741}, 95\% CI $[-0.033,+0.032]$. To within ${\approx}3$
points, a person does not read deception better than a keyword list. \tbf{(2) The
...
$h$ on those, the quality rate on the rounds I passed is
$q_{\text{declined}}{=}(p-bh)/(1-b)$, since the two strata must reconstitute the
prior. That answers only the aggregate question ``am I leaking in this cell'',
